      ...
      if ($entity_type_id === 'file') {
        foreach ($change_list['field_storage_definitions'] as $field_name => $change) {
          if ($change === EntityDefinitionUpdateManagerInterface::DEFINITION_DELETED) {
            ...
            assert(is_string($base_column));
            if (isset(\Drupal::service('filehash')::getAlgorithms()[$base_column])) {
              $definition = \Drupal::entityDefinitionUpdateManager()->getFieldStorageDefinition($field_name, 'file');
              assert(isset($definition));
              $columns[$field_name] = $definition->getLabel();
            }
          }
        }
      }
